...
@Component({
  selector: "midas-balance-account-group",
  templateUrl: "./balance-account-group.component.html",
  styleUrls: ["./balance-account-group.component.scss"],
  animations: [
...
export class BalanceAccountGroupComponent implements OnInit {
  expandedElement: any;
  ...
  constructor(
    ...
    this.groupsService.getBalanceAccountOfGroup().subscribe((result) => {
      const { permissions, staffId } = this.currentUser;
      this.dataSource = result?.result?.listBalanceGroup.filter((v: any) => Number(v.account_balance_derived) >= 0);
      this.dataSource2 = result?.result?.listBalanceGroup.filter((v: any) => Number(v.account_balance_derived) < 0);
      ...
      this.loadData();
      this.loadData2();
      this.totalAmountDerived = this.accountFilter.reduce(
        (sum, { account_balance_derived }) => sum + account_balance_derived,
        0
      );
      this.totalAmountDerived2 = this.accountFilter2.reduce(
        (sum, { account_balance_derived }) => sum + account_balance_derived,
        0
      );
  ...
  ngAfterViewInit() {
    ...
    merge(this.table2Sort.sortChange, this.table2Paginator.page)
      .pipe(tap(() => this.loadData2()))
      .subscribe();
  }

  ngOnInit(): void {
    this.formFilter.valueChanges.subscribe((e) => [this.filterData()]);
    this.formFilter2.valueChanges.subscribe((e) => [this.filterData2()]);
  }
