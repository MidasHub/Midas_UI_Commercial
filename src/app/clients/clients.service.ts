...
export class ClientsService {
  ...
  addClientDatatableEntry(clientId?: string | null, datatableName?: string, data?: any): Observable<any> {
    const httpParams = new HttpParams().set('genericResultSet', 'true');
    if (clientId && datatableName) {
  ...
  deleteDatatableContent(clientId?: string|null, datatableName?: string) {
    const httpParams = new HttpParams().set('genericResultSet', 'true');
    if ((clientId) && (datatableName)) {
  ...
  getClientSignatureImage(clientId: string, documentId: string) {
    const httpParams = new HttpParams().set(
      ...
      window.localStorage.getItem('Fineract-Platform-TenantId') ||
        window.sessionStorage.getItem('Fineract-Platform-TenantId') ||
        ''
    );
  ...
  addClientIdentifier(clientId?: string | null, identifierData?: any) {
    if ((clientId) && (identifierData)) {
  ...
  uploadClientDocument(clientId?: string|null, documentData?: any) {
  ...
  createClientAddress(clientId?: string | null, addressTypeId?: string, addressData?: any) {
    return this.http.post(`/client/${clientId}/addresses?type=${addressTypeId}`, addressData);
  }

  editClientAddress(clientId?: string | null, addressTypeId?: string, addressData?: any) {
  ...
  ToggleStatusICPartner(idIC: any, status: any): Observable<any> {
    let httpParams = this.commonHttpParams.getCommonHttpParams();
    httpParams = httpParams.set('externalPartnerId', idIC).set('toggleStatus', status);
    return this.http.post<any>(`${this.IcGatewayApiUrlPrefix}/client/togglestatusicpartner`, httpParams);
  }

  DeleteICPartner(idIC: any): Observable<any> {
    let httpParams = this.commonHttpParams.getCommonHttpParams();
    httpParams = httpParams.set('externalPartnerId', idIC);
