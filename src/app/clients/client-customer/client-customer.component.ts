...
export class ClientCustomerComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit() {
    ...
    this.formFilter.get('officeId')?.valueChanges.subscribe((result: any) => {
      ...
      this.centersService.getStaff(result).subscribe((staffData: any) => {
        this.staffs = staffData?.staffOptions?.filter((staff: any) => staff.displayName.startsWith('R'));
        this.staffs.unshift({
          id: null,
          displayName: 'Tất cả',
        });
