...
export class BalanceAccountClientComponent implements OnInit {
  ...
  constructor(
    ...
    this.clientsServices.getBalanceAccountOfCustomer().subscribe(result => {
      this.dataSource = result?.result?.listBalanceCustomer;
      this.accountFilter = this.dataSource;
      this.loadData();
    });
    this.clientsServices.getNameOfStaff().subscribe(result => {
  ...
  sortData(sort: any) {
    this.accountFilter = this.accountFilter.sort((a, b) => {
      ...
      switch (sort.active) {
        ...
        case 'staff_name': return this.compare(a.staff_name, b.staff_name, isAsc);
        case 'account_no': return this.compare(a.account_no, b.account_no, isAsc);
        case 'account_balance_derived': return this.compare(a.account_balance_derived, b.account_balance_derived, isAsc);
        default: return 0;
      }
    });
