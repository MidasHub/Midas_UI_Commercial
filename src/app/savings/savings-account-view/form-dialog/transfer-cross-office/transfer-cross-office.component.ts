...
export class TransferCrossOfficeComponent implements OnInit {
  ...
  modifiedForm() {
    ...
    if (value === 58) {
      ...
      this.form.get('partner')?.valueChanges.subscribe((partnerValue) => {
        this.savingsService.getListICSavingAccountByPartner(partnerValue).subscribe((result) => {
      ...
      if (value === 59) {
        ...
        if (value === 60) {
          ...
          this.form.get('office')?.valueChanges.subscribe((officeValue) => {
            this.serviceClient.getStaffsByOffice(officeValue).subscribe((result) => {
              this.staffs = result.result.listStaff;
            });
          });

          this.form.get('client')?.valueChanges.subscribe((clientValue) => {
            this.savingsService.getListClientSavingStaffByOffice(clientValue).subscribe((result) => {
              this.accounts = result.result.listClientSavingVault;
            });
          });
        }
