  ...
  submitTransactionCash() {
    ...
    dialog.afterClosed().subscribe((data) => {
      if (data) {
        ...
        if (this.transactionInfo.type === 'cash') {
          this.transactionService.submitTransactionCash(this.transactionInfo).subscribe((txnData: any) => {
            if (!data.error) {
              this.afterSuccessCreateCashTransaction(txnData);
            } else {
              this.alertService.alert({
                message: data.error,
                msgClass: 'cssDanger',
          ...
          if (this.transactionInfo.type === 'rollTermGetCash') {
            this.transactionService
              ...
              .subscribe((txnData: any) => {
                if (!txnData.error) {
                  this.afterSuccessCreateCashTransaction(txnData);
                } else {
                  this.alertService.alert({
                    message: txnData.error,
                    msgClass: 'cssDanger',
                    hPosition: 'center',
                  });
                  return;
  ...
  submitTransactionRollTerm() {
    ...
    dialog.afterClosed().subscribe((data) => {
      if (data) {
        ...
        const listBookingRollTerm = JSON.stringify(this.listRollTermBooking,  (key, value) => {
          if (key === '$$hashKey') {
            return undefined;
          }
          return value;
        ...
        this.transactionService.submitTransactionRollTerm(this.transactionInfo).subscribe((txnData: any) => {
          this.listRollTermBooking = [];
          this.transactionInfo.transactionRefNo = txnData.result.tranRefNo;
          this.transactionInfo.transactionId = txnData.result.id;
          // this.transactionInfo.isDone = true;
    ...
    this.savingsService.getSavingsAccountTemplate(clientId, productId, false).subscribe((response: any) => {
      this.savingsService.createClientSavingsAccount(clientId, productId, response).subscribe((savingsAccount: any) => {
        ...
        this.savingsService.executeSavingsAccountCommand(savingsAccount.savingsId, 'approve', data).subscribe(() => {
          ...
          this.savingsService.executeSavingsAccountCommand(savingsAccount.savingsId, 'activate', txnData).subscribe(() => {
